...
export const useLayoutAlgorithm = () => {
  ...
  return React.useMemo(() => {
    ...
    const { deviceList, instanceList } = devicesFullQuery.data;

    const elk = new ELK();

    return async () => {
      const cardsSelector = deviceList
        .map(({ id }) => `[data-id="${id}"]`)
        .join(",");
      const cards = Array.from(
        document.querySelectorAll<HTMLElement>(cardsSelector)
      );
      ...
      const edges = netEntityLinks.data
        ...
        .map((link): ElkExtendedEdge => {
          const getId = (entityId: string) => {
            if (deviceList.some((device) => device.id === entityId)) {
              return entityId;
            }

            const idFromInstance = instanceList.find(
              (instance) => instance.id === entityId
            )?.parentId;
            if (idFromInstance) {
