...
export const DeviceNodeHandles: React.FC<NodeProps<DeviceNodeType>> = ({
  data,
  width,
  height,
  ...rest
  ...
  const deviceIds = deviceList.flatMap((device) => {
    const allIds = [
      device.id,
      ...instanceList
        .filter((instance) => instance.parentId === device.id)
  ...
  const getPositionAndStyle = ({
    ...
    console.error("layout: GetPosition failure", {
      id: rest.id,
      x,
      y,
      relativeX,
  ...
  const handles = netEntityLinks.data.map((link) => {
    const renderSourceHandle = () => {
      ...
      const startPoint = data.sources?.find(
        (value) => value.id === link.id + "_s0"
      )?.startPoint;
    ...
    const renderTargetHandle = () => {
      ...
      const endPoint = data.targets?.find(
        (value) => value.id === link.id + "_s0"
      )?.endPoint;
